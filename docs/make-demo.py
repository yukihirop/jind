#!/usr/bin/env python3
"""docs/demo.svg を作る。入力は pty で取った実際の jind 出力(ANSI 付き)の JSON:
[{"cmd": "jind ...", "out": "<raw bytes as str>"}, ...]
...
def main(path, at=None):
    ...
    for si, sc in enumerate(scenes):
        ...
        while lines and lines[-1].strip() == "":
            lines.pop()
        # jev を待つ場面は find が出るまで少し空く
        first_wait = 0.7 if "jev:" in out else 0.35
        t += first_wait
        for ln in lines:
            if rows >= ROWS:
                break
    ...
    out.append(f'<text x="{W / 2}" y="21" text-anchor="middle" font-size="12" fill="{DIM}">jind — jev × find</text>')
    out.extend(els)
    out.append("</svg>")
    sys.stdout.write("\n".join(out))
