...
fn run(parsed: cli::Parsed, typed: &str) -> Result<i32, JindError> {
    ...
    if words[0] == "demo" {
        ...
        eprintln!("{}", paint(on, C::Dim, &format!("sample tree in {} (rebuilt each run)", root.display())));
...
fn execute(opts: &cli::Opts, words: Vec<String>, passthrough: Vec<String>, typed: &str) -> Result<i32, JindError> {
    let cfg = config::load()?;
