...
pub fn explain_text(tokens: &[Token], jev: Option<&JevInfo>, on: bool) -> String {
    ...
    for t in tokens {
        ...
        let role_c = match t.role {
            ...
            Some(
                Role::NamePattern | Role::Extension | Role::NameWord | Role::Type | Role::Empty,
            ) => C::Green,
        ...
        let role = format!(
            "{}{}",
            paint(on, role_c, &role_s),
            " ".repeat(16usize.saturating_sub(role_s.len()))
        );
...
pub fn confirm(prompt: &str) -> Choice {
    ...
    eprint!(
        "{} {} ",
        paint(on, C::Yellow, prompt),
        paint(on, C::Dim, "[Y/n/e]")
    );
...
fn wait_flag(prog: &str) -> Option<&'static str> {
    let name = std::path::Path::new(prog)
        ...
        .unwrap_or(prog);
    match name {
        "code" | "code-insiders" | "codium" | "cursor" | "windsurf" | "subl" | "zed" | "atom"
        | "mate" => Some("--wait"),
        "bbedit" => Some("-w"),
        _ => None,
    }
...
pub fn edit_command(
    ...
    let mut words =
        shell_words::split(&editor).map_err(|e| JindError::Usage(format!("bad $EDITOR: {e}")))?;
    if words.is_empty() {
