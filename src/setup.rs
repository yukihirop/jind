...
pub fn run() -> Result<i32, JindError> {
    let path = crate::config::path().ok_or_else(|| JindError::Config("cannot determine config path (HOME unset)".into()))?;
    let existing = if path.exists() { std::fs::read_to_string(&path)? } else { String::new() };
    let mut table: toml::Table = if existing.trim().is_empty() {
        toml::Table::new()
    } else {
        toml::from_str(&existing).map_err(|e| JindError::Config(format!("{}: {e}", path.display())))?
    };
    ...
    if !std::io::stdin().is_terminal() {
        return Err(JindError::Usage("jind setup needs a terminal to read the key".into()));
    }
    ...
    let jev_tbl = jev_tbl.as_table_mut().ok_or_else(|| JindError::Config("[jev] is not a table".into()))?;
