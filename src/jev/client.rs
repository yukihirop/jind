    ...
    fn decide(&self, state: Value, questions: Questions) -> Result<DecisionsResponse, JindError> {
        ...
        loop {
            let res = agent
                ...
                .header("HTTP-Referer", "https://github.com/yukihirop/jind")
                .header("X-Title", "jind")
                .send_json(&body);
            let mut res = match res {
                Ok(r) => r,
                Err(e) => return Err(JindError::Jev(format!("request failed: {e}"))),
            };
            ...
            if (200..300).contains(&status) {
                return res
                    ...
                    .map_err(|e| JindError::Jev(format!("bad response: {e}")));
            }
