...
**jind** turns a loose pile of words — out of order, half-remembered, misspelled — into the `find` command you meant, shows it, and runs it.
...
Sister project of [jurl](https://github.com/yukihirop/jurl) (jev × curl); same shape, same jev client.

## How it works

`words → rules → (jev if anything is left) → repair → find → confirm → run`

- **rules** decide the unambiguous shapes in code: paths, globs, `files`/`dirs`, `delete`/`ls`/`count`, `+7d`, `>10M`, `days`, `except`, `depth`, `today`. If every word resolves, jind runs offline with no prompt.
- **jev** ([TypeSafe System One](https://openrouter.ai), via OpenRouter) gets the rest in **one request**: "what is the role of each word?" plus, for numbers, "which unit?" and "at least or at most?". jev only picks from fixed choices and returns probabilities; it never generates the command.
...
- **delete** always asks, always with the default on *No*, and always after listing what would go — the same expression is run without `-delete` first.
...
`OPENROUTER_API_KEY` in the environment takes precedence; a key saved by `jurl setup` is picked up too. `find` must be on `PATH` (BSD and GNU both work). Tested on macOS.
